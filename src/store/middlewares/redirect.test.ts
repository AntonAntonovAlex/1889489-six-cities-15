...
describe('Redirect middleware', () => {
  ...
  it('should not redirect to "/NotFoundScreen" with empty action', () => {
    const emptyAction = { type: '', payload: AppRoute.NotFoundScreen };
    store.dispatch(emptyAction);
    expect(browserHistory.location.pathname).not.toBe(AppRoute.NotFoundScreen);
  });
});
